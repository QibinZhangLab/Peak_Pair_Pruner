...
# str_is_number_os_or_neg() checks that a user-provided string is a number (sign is irrelevant).
...
def call_ppp(area_directory, mdf_bot, mdf_top, light_lth_r, heavy_htl_r, mix_lth_r, mix_lth_tol, tags, tag_shift, tag_tol, delete_multi, subtract_b, subtract_nh, processed_name, format_menu_choice, destination_directory):
    ...
    # The booleans below mostly relate to checking that the user has uploaded a correctly formatted matrix file.
    ...
    if not no_ppp:
        ...
        print('')
        print('Potential peak pairs from MS-DIAL alignment matrix:')
        ...
        if format_choice == 'Matrix':
            ...
            with open(name_and_directory, 'a') as f:
                ...
                f.write('END\n')
                f.write('PPP v1.0 modified alignment matrix with the following PPP parameters:' + '\n')
            ...
            peakpair_columns = ['Metabolite', 'Alignment ID', 'Average RT (min)', 'Average MZ (Light)', 'Average MZ (Heavy)', 'Adduct', 'ID type', 'Total Score']
            ...
            row = 0
            worksheet.write_row(row, 0, ['PPP v1.0 report with the following PPP parameters:'])
            row += 1
            row += 1
            ...
            row += 1
            worksheet.write_row(row, 0, ['Number of tags per molecule: ' + str(tag_count)])
            row += 1
            ...
            for peak_pair in identified_peak_sets:
                col = 0
            ...
            for peak_pair in unknown_peak_sets:
                col = 0
                metabolite = peak_pair[0][metabolite_head_pos-1]
                if metabolite == 'Unknown':
                    is_unknown = True
...
def main_function():
    ...
    while not exit_ppp:
        if analyze:
            ...
            print('Working...')
            print('')
            analyze = call_ppp(area_directory, mdf_range_bottom, mdf_range_top, light_lth_ratio, heavy_htl_ratio, mix_lth_ratio, mix_lth_tolerance, tag_count, tag_isotopic_shift, tag_isotopic_shift_tolerance, delete_multimatches, subtract_blank, subtract_natural_heavy, processed_name, format_menu_choice, destination_directory)
            print('')
            print('Finished.')
        ...
        layout = [
                    [sG.Text('An Addon to MS-DIAL for Peak Pair Validation and Ratio Quantification of Isotopic Labeling LC-MS(/MS) Data', font='Arial 11 bold')],
                    [sG.Text('MS-DIAL alignment matrix directory\\name: ' + '\t' + '\t'), sG.InputText(area_directory), sG.FileBrowse(file_types=(("Text Files", "*.txt"),))],
        ...
        while True:
            ...
            if event == 'GO':
                ...
                if not exists(values[j]):
                    analyze = False
                    print('Error: MS-DIAL alignment matrix not selected or not found.')
                j += 1
                ...
                if not str_is_number(values[j]):
                    analyze = False
                    print('Error: number of tags per molecule should be a unitless integer >= 0 without spaces.')
                elif not str_is_integer(values[j]):
                    analyze = False
                    print('Error: number of tags per molecule should be a unitless integer >= 0 without spaces.')
                else:
                    tag_count = int(values[j])
                j += 1
                ...
                # A multi-match is a set of 3 or more peaks with overlapping pairings (a theoretical MS-DIAL error).
                # Multi-matches have not been encountered and so have not been thoroughly tested.
                # As processing multi-matches has not been thoroughly tested, they will be simply be deleted for now.
